/obj/item/deployable_optable
	...
	var/deployable_item = /obj/machinery/deployable/optable

/obj/item/deployable_optable/Initialize()
	. = ..()
	AddComponent(/datum/component/deployable_item, deployable_item, 10 SECONDS, 20 SECONDS)

/obj/machinery/deployable/optable
...
/obj/machinery/deployable/optable/Initialize(mapload)
	. = ..()
...
/obj/machinery/deployable/optable/examine(mob/user)
	. = ..()
...
/obj/machinery/deployable/optable/attack_hand(mob/living/user)
	. = ..()
	if(.)
		return
	if(anes_tank)
...
/obj/machinery/deployable/optable/user_buckle_mob(mob/living/buckling_mob, mob/user, check_loc = TRUE, silent)
...
/obj/machinery/deployable/optable/proc/knock_out_buckled(mob/living/buckled_mob)
	if(!victim || victim != buckled_mob)
		return
	ADD_TRAIT(buckled_mob, TRAIT_KNOCKEDOUT, OPTABLE_TRAIT)

/obj/machinery/deployable/optable/user_unbuckle_mob(mob/living/buckled_mob, mob/user, silent)
	. = ..()
	if(!.)
		return
	if(!silent)
...
/obj/machinery/deployable/optable/post_unbuckle_mob(mob/living/buckled_mob)
...
/obj/machinery/deployable/optable/proc/remove_knockout(mob/living/buckled_mob)
	REMOVE_TRAIT(buckled_mob, TRAIT_KNOCKEDOUT, OPTABLE_TRAIT)

/obj/machinery/deployable/optable/MouseDrop_T(atom/A, mob/user)
...
/obj/machinery/deployable/optable/proc/check_victim()
	if(locate(/mob/living/carbon/human, loc))
		var/mob/living/carbon/human/M = locate(/mob/living/carbon/human, loc)
		if(M.lying_angle)
			victim = M
			icon_state = M.handle_pulse() ? "table2-active" : "table2-critical"
			return 1
	victim = null
	stop_processing()
...
/obj/machinery/deployable/optable/process()
	check_victim()

/obj/machinery/deployable/optable/proc/take_victim(mob/living/carbon/C, mob/living/carbon/user)
	if (C == user)
	...
	if(ishuman(C))
		...
		victim = H
		start_processing()
		icon_state = H.handle_pulse() ? "table2-active" : "table2-critical"
	else
		icon_state = "table_deployable_deployed"

/obj/machinery/deployable/optable/verb/climb_on()
...
/obj/machinery/deployable/optable/attackby(obj/item/I, mob/user, params)
	. = ..()
...
/obj/machinery/deployable/optable/proc/check_table(mob/living/carbon/patient as mob)
	if(victim)
