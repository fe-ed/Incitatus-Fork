		...
		switch(state)
			if(OW_MAIN)
				for(var/datum/squad/S AS in watchable_squads)
					dat += "<b>[S.name] [S.additional_name]</b> <a href='?src=\ref[src];operation=message;current_squad=\ref[S]'>\[Message Squad\]</a><br>"
					if(S.squad_leader)
					...
					dat += "<A href='?src=\ref[src];operation=monitor[S.id]'>[S.name] [current_squad.additional_name] Monitor</a><br>"
				dat += "----------------------<br>"
			...
			if(OW_MAIN)
				for(var/datum/squad/S AS in watchable_squads)
					...
					if(S.overwatch_officer)
						...
						dat += "<b>Squad Overwatch:</b> <font color=red>NONE</font><br>"
					dat += "<A href='?src=\ref[src];operation=monitor[S.id]'>[S.name] [S.additional_name] Monitor</a><br>"
			if(OW_MONITOR)//Info screen.
				dat += get_squad_info()
