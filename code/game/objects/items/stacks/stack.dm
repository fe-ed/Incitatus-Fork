...
/obj/item/stack/proc/create_object(mob/user, datum/stack_recipe/R, multiplier)
	...
	if(R.skill_req && user.skills.getRating(SKILL_CONSTRUCTION) < R.skill_req)
		building_time += R.time * ( R.skill_req - user.skills.getRating(SKILL_CONSTRUCTION) ) * 0.2 // +20% time each skill point lacking. //RUTGMC EDIT 50% > 20%
	if(R.skill_req && user.skills.getRating(SKILL_CONSTRUCTION) > R.skill_req)
		building_time -= clamp(R.time * ( user.skills.getRating(SKILL_CONSTRUCTION) - R.skill_req ) * 0.20, 0 , 0.85 * building_time) // -20% time each extra skill point //RUTGMC EDIT 40% > 20%
	if(building_time)
